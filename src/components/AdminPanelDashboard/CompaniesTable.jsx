...
export default function CompaniesTable({ isDarkMode, onNavigateSection }) {
    ...
    <main className={`${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} p-4 sm:p-6 min-h-screen`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <button
            onClick={() => downloadCSV('companies.csv', filtered)}
          ...
          <table className="min-w-full table-auto border-collapse">
            <thead className={`${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
              <tr className={`text-left border-b ${isDarkMode ? 'border-gray-700' : ''}`}>
                ...
                <th className={`px-4 py-3 ${isDarkMode ? 'text-gray-100' : ''}`}>Company Name</th>
                <th className={`px-4 py-3 ${isDarkMode ? 'text-gray-100' : ''}`}>Email</th>
              </tr>
            </thead>
            <tbody>
              {pageData.length === 0 ? (
                <tr><td colSpan={3} className="p-4 text-center">No companies found.</td></tr>
              ) : pageData.map((c) => (
